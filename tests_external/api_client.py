...
class LEDControllerAPI:
    ...
    def get_modes(self) -> List[str]:
        ...
        response = self.session.get(f"{self.base_url}/modes")
        response.raise_for_status()
        return response.json()

    def get_current_mode(self) -> str:
        """Get the current mode."""
        return self._get("/modes/current")["mode"]

    def set_mode(self, mode: str) -> str:
        """Set the current mode."""
        return self._post("/modes/current", {"mode": mode})["mode"]
    ...
    def set_brightness(self, value: int) -> int:
        ...
        return self._post("/brightness", {"brightness": value})["brightness"]

    # Delay endpoints
    def get_delay(self) -> float:
        """Get current delay."""
        return self._get("/delay")["delay"]

    def set_delay(self, value: float) -> float:
        """Set delay in seconds."""
        return self._post("/delay", {"delay": value})["delay"]

    # Options endpoints
    def get_opts(self) -> Dict[str, Any]:
        """Get current mode options."""
        return self._get("/opts")["opts"]

    def set_opts(self, opts: Dict[str, Any]) -> Dict[str, Any]:
        """Set mode options."""
        return self._post("/opts", opts)["opts"]

    # Strip info
    def get_strips(self) -> List[Dict[str, Any]]:
        """Get information about configured LED strips."""
        return self._get("/strips")["strips"]
